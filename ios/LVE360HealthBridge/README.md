...
4. Add `lve360-health://auth/callback` to Supabase Auth's allowed redirect URLs for the same project used by the web dashboard. Google must remain enabled in Supabase Auth.
...
- Sign in with the same private LVE360 account used on the web. A different account must not inherit the first account's chosen categories or anchors.
