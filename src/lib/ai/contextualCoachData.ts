  ...
  return [
    {
      ...
      content: [
        ...
        "proposed_action must be null unless the intent is BEHAVIORAL_COACHING, PRIORITIZATION, or PROGRESS_COACHING and one safe lifestyle practice directly follows from the answer. It must be non-null when the member explicitly asks for a next-week habit or practice and the answer provides one safe lifestyle action. When eligible, use {type:'weekly_practice', identity_direction, action_label, cue, frequency_per_week, minimum_version, rationale}. identity_direction must be one of movement, nutrition, sleep, emotional_health, relationships, focus, career, happiness, overall_health. Never put a medication, hormone, supplement, dose, test, clinician task, reminder, or saved-record change in proposed_action.",
      ].filter(Boolean).join(" "),
    },
    {
      role: "user" as const,
