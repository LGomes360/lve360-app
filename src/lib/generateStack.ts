...
• “Recommended Stack” MUST be a Markdown table; mark items also in *Current Stack* with **(already using)**, then add paragraph **“Synergy & Timing”**.  
• Each supplement (sections 5-7) needs ≥1 clickable PubMed/DOI URL citation.  
• Summary greets first name, second person, 1 emoji max.  
• Finish with \`## END\`.  If any rule is unmet, regenerate internally.`;
}
...
function blueprintTableOK(md: string) {
  const blk = md.match(/## High-Impact[\s\S]*?\n\|/i);
  if (!blk) return false;
  const rows = blk[0].split("\n").filter((l: string) => l.startsWith("|"));
  return rows.length >= MIN_BP_ROWS + 1;
}
function ensureEnd(md: string) {
  return hasEnd(md) ? md : md + "\n\n## END";
}

// harvest first ≤10 recs from Recommended section
function harvestRecs(md: string) {
  ...
  return sec[1]
    ...
    .filter(Boolean)
    .filter(
      (l: string) => l.startsWith("|") || l.startsWith("-") || /^\d+\./.test(l)
    )
    .slice(0, MIN_BP_ROWS)
    .map((l: string) => {
      if (l.startsWith("|")) return l.split("|")[1].trim();
      return l.replace(/^[-\d.]+\s*/, "").split(/\s{2,}/)[0];
    });
}
...
function injectBlueprint(md: string) {
  ...
  const table =
    [
      ...
      "| ---- | ---------- | -------------- |",
      ...names.map(
        (n: string, i: number) => `| ${i + 1} | ${n} | Auto-added placeholder |`
      ),
      "",
      "**Why these 10 matter:**",
      "These picks give the biggest health return per dollar and complement your regimen.",
      "",
    ].join("\n") + "\n";
...
function ensureRecTable(md: string) {
  ...
  return md.replace(
    ...
    (_: string, body: string, endMarker: string) => {
      const lines = body
        .split("\n")
        .map((l: string) => l.trim())
        .filter(Boolean)
