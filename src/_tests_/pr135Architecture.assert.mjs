...
const [memberData, intention, todayBrief, weeklyRoute, weeklyData, coach, coachFallback, memberReportedContext, dailyLog, logsRoute] = await Promise.all([
  ...
  readFile("src/lib/coachFallback.ts", "utf8"),
  readFile("src/lib/memberReportedContext.ts", "utf8"),
...
assert.match(coachFallback, /You recorded that/, "a rejected AI draft must fall back to attributed member context");
assert.match(coachFallback, /sourceIds: \["recent_check_ins"\]/, "the grounded fallback must cite the saved check-in");
assert.match(memberReportedContext, /requestsMemberReportedContext/, "routing, validation, and fallback should share one context-request detector");
