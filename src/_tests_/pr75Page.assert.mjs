...
const webhookReliability = read("src/lib/reminderWebhook.ts");
...
assert.match(webhook, /verifyReminderProviderEvent/, "Resend events must be signature verified.");
assert.match(webhookReliability, /new Webhook\(secret\)\.verify/, "The verification helper must use Svix signatures.");
assert.match(webhookReliability, /email\.delivered/, "Delivered reminder status must be observable.");
assert.match(webhookReliability, /email\.bounced/, "Bounced reminder status must be observable.");
