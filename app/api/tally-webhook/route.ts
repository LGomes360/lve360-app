...
// LVE360 // API Route (2025-10-12 ENHANCED A.6)
// Handles incoming Tally webhook, normalizes/validates data, creates/links user,
// UPSERTS submission (idempotent), logs failures, and triggers generate-stack.
...
function hmacValid(rawBody: string, headerSig: string | null): boolean {
  if (!TALLY_HMAC_SECRET || !headerSig) return true; // skip if not configured
  const digest = crypto
    .createHmac("sha256", TALLY_HMAC_SECRET)
    .update(rawBody, "utf8")
    .digest("hex");
  // Tally may send bare hex or "sha256=..."
  const incoming = headerSig.replace(/^sha256=/, "");
...
export async function POST(req: NextRequest) {
  ...
  try {
    rawBody = await req.text(); // to support HMAC calc
    try {
      body = JSON.parse(rawBody);
    } catch {
      // fallback: if body was already parsed upstream
      body = (rawBody as any) || {};
    }

    // Optional signature + replay protection
    const headerSig = req.headers.get("x-tally-signature") || req.headers.get("tally-signature");
    if (!hmacValid(rawBody, headerSig)) {
    ...
    const createdAt = Number(new Date(body?.createdAt ?? Date.now())) || Date.now();
    const now = Date.now();
    if (Math.abs(now - createdAt) / 1000 > REPLAY_TTL_SECONDS) {
      await supa.from("webhook_failures").insert({
        source: "tally",
        event_type: body?.eventType ?? null,
        event_id: body?.eventId ?? null,
        error_message: "replay_window_exceeded",
        severity: "warning",
        payload_json: safeJson(body),
      });
      // Non-fatal: continue, but you may choose to reject here
    }
    ...
    const tally_submission_id: string | null =
      body?.data?.submissionId || body?.id || null;
    ...
    // Strict Zod validation (surface all issues)
    ...
    // Find or create user (canonicalize by email)
    ...
    if (data.user_email) {
      ...
      if (userRow?.id) {
        ...
        // Try getting last submission's user_id for same email
        const { data: lastSub } = await supa
          .from("submissions")
        ...
        if (lastSub?.user_id) insertPayload.id = lastSub.user_id;
    ...
    const { data: subRow, error: subErr } = await supa
      .from("submissions")
      .upsert(submissionRow, { onConflict: "tally_submission_id" })
      .select("id")
      .single();
