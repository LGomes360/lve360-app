...
function isStripeCustomer(
  ...
  if (!c) return false;
  // DeletedCustomer has { deleted: true }; Customer has no 'deleted' or it's not true
  const anyC = c as any;
  if (typeof anyC.deleted === "boolean") return anyC.deleted === false;
  return true;
}
...
export async function GET(req: Request) {
  try {
    ...
    // 1) Retrieve session with subscription & customer expanded
    ...
    // 2) Resolve the *target* user id WITHOUT cookies
    let targetUserId =
    ...
    // 4) Try simple upsert-by-id first
    ...
    // 5) Reconcile duplicate email if needed (unique constraint 23505)
    if (ensure.error?.code === "23505" && emailLower) {
      ...
      // A) Both exist, different ids -> free email on dup, update target, delete dup
      if (byEmail?.id && byId?.id && byEmail.id !== byId.id) {
        const clr = await supabaseAdmin.from("users").update({ email: null }).eq("id", byEmail.id);
        if (clr.error) {
          console.error("[stripe/confirm] clear dup email failed:", clr.error);
        ...
        const upd = await supabaseAdmin
          ...
          .maybeSingle();
        if (upd.error) {
          console.error("[stripe/confirm] update target failed:", upd.error);
        ...
        if (del.error) console.warn("[stripe/confirm] delete dup non-fatal:", del.error);

        ensure = { data: upd.data, error: null } as any;
      }
      // B) Email row exists, no row for target id -> try re-key; else clear+insert
      else if (byEmail?.id && !byId?.id) {
        const rekey = await supabaseAdmin.from("users").update({ id: targetUserId }).eq("id", byEmail.id);
        if (rekey.error) {
          ...
          const ins = await supabaseAdmin
            ...
            .maybeSingle();
          if (ins.error) {
            console.error("[stripe/confirm] fallback insert failed:", ins.error);
      ...
      // C) Only target-id row exists or same row -> direct update
      ...
      // D) Nothing matched -> insert without email, then try to set it
      else {
        const ins2 = await supabaseAdmin
          ...
          .maybeSingle();
        if (ins2.error) {
          console.error("[stripe/confirm] last-resort upsert failed:", ins2.error);
          return NextResponse.json({ ok: false, error: "last-resort-upsert-failed" }, { status: 500 });
        }
        const setEmail = await supabaseAdmin.from("users").update({ email: emailLower }).eq("id", targetUserId);
        if (setEmail.error) console.warn("[stripe/confirm] could not set email (non-fatal):", setEmail.error);
