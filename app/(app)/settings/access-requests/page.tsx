...
type RequestRow = {
  ...
  status: "pending" | "waitlisted" | "declined";
...
export default async function AccessRequestsPage() {
  ...
  const { data, error } = await admin
    ...
    .select("id,email,first_name,organizing_help,interests,request_reason,referral_source,referral_code,status,founder_notes,created_at")
    .in("status", ["pending", "waitlisted", "declined"])
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Review and triage the private-membership queue. Approval and account admission remain locked until PR166 adds single-use invitation issuance and the founder GO gate is explicitly opened.</p>
      </div>
        ...
        <div className="space-y-4">
          {requests.map((request) => (
            <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" key={request.id}>
              ...
              <AccessRequestActions id={request.id} initialNotes={request.founder_notes} initialStatus={request.status} />
            </article>
          ))}
        </div>
      )}
